...
const AddBanner = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${apiurl}/admin/banner/`, payload, {
        headers: {
          Authorization: token,
        },
      });
  ...
  return (
    <Layout>
      <Container>
        {loading && (
          <DynamicLoader maintext="Please wait" subtext="Saving Banner" />
        )}
        <ToastContainer />
        <div className="flex flex-wrap justify-between w-full h-full overflow-auto">
          <div className="flex flex-col py-2 px-2 w-full">
            <BackHeader title="Add Banner" backButton={true} link="/banner" />
            <div className="p-6">
