...
const ImageMapping = () => {
  ...
  const fetchServiceList = async () => {
    setloading(true);
    try {
      const response = await axios.get(`${apiurl}/admin/Image/list`, {
        headers: {
          Authorization: token,
          "ngrok-skip-browser-warning": "69420",
        },
      });
      console.log("service image/list", response);
  ...
  const handleDeleteImageService = async (id) => {
    try {
      const response = await axios.delete(`${apiurl}/admin/image/${id}`);
      if (response.data.success === true) {
        toast.success("Image service deleted successfully");
        fetchServiceList();
      }
    } catch (error) {
                          ...
                          <td>
                            <span className="flex gap-1 cursor-pointer">
                              <CiEdit
                                ...
                                onClick={() => handleEditStatus(item?.imageId)}
                              />
                              <MdDeleteForever
                                ...
                                onClick={() =>
                                  handleDeleteImageService(item?.imageId)
                                }
                              />
                            </span>
                          </td>
