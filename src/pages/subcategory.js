...
const Subsubcategory = () => {
  ...
  const { currentPage = 1, itemsPerPage = 10, totalItems, totalPages } = pagination;
  ...
  const handleSearch = (value) => {
    ...
    if (!value.trim()) {
      setCurrentPage(1);
      setserchData(subcategories);
      return;
    }

    // Filter store data based on store name (handle case sensitivity)
    const filteredData = subcategories.filter(
      (store) => store?.name?.toLowerCase().includes(value.toLowerCase()) || store?.description?.toLowerCase().includes(value.toLowerCase()) ||  store?.category?.name?.toLowerCase().includes(value.toLowerCase()));
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${apiurl}/admin/subcategory/importSubCategories`,
        formData,
        {
          headers: {
            Authorization: token,
      ...
      fetchCategoryList()
      toast.success("Sub Category imported successfully");
      setIsOpen(false);
  ...
  return (
    <Layout>
      <Container>
        {loading == true ? <DynamicLoader maintext="wait" subtext="Fetching subcategory Data" /> : null}
          ...
          <div className="flex flex-col  py-2 px-2 w-full">
              ...
              rightSide={
                <div className="flex gap-3 w-[650px]">
                 {/* <button
                    ...
                    className="bg-[#000] hover:bg-[#e7c984] hover:text-[#000] text-white min-w-[180px] py-2 px-4 rounded-md shadow-md  transition whitespace-nowrap"
                  >
                    Import Sub Category
                  </button> */}
                  <Link to="/addsubcategory"><button className="bg-[#000] hover:bg-[#e7c984] hover:text-[#000] text-[#fff] w-[150px] p-2 rounded-md">Add Sub Category</button></Link>

                  <Input.search placeholder="Search Sub Category "
                    value={searchInput}
            ...
            {editDetails && (
              <Card.AddPopUp removePopUp={() => setEditDetails("")}>
                <div className="p-4">
                  ...
                  <p>
                    <strong>subcategory:</strong> {editDetails.subcategory}
                  </p>
                  <p>
            ...
            <div className="relative overflow-x-auto shadow-md sm:rounded-lg mt-5">
              <table className="w-full text-sm text-left rtl:text-right text-gray-500">
                ...
                <tbody>
                  {serachData &&
                    serachData?.map((subcategory, index) => {
